...
public:
  IndexIterator(
      page_id_t page_id,
      int idx,
      BufferPoolManager &buff);

  ~IndexIterator();

  IndexIterator(const IndexIterator &from)
      : IndexIterator(
            from.leaf_page_->GetPageId(),
            from.pos_,
            from.buffer_pool_)
  {
  }
  ...
  B_PLUS_TREE_LEAF_PAGE_TYPE *GetLeafPage(
      page_id_t page_id)
  {
    ...
    return reinterpret_cast<B_PLUS_TREE_LEAF_PAGE_TYPE *>(
        buffer_pool_.FetchPage(page_id)->GetData());
  }
