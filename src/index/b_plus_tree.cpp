                              ...
                              Transaction *transaction)
{
  ...
  auto leaf = FindLeafPage(key, transaction, TreeOperation::Search);
  result.resize(1);
  const bool ret = leaf->Lookup(key, result[0], comparator_);
  if (transaction)
  {
    ReleaseAllLatches(transaction, TreeOperation::Search, false);
  }
  else
  {
    ...
    Transaction *transaction)
{
  ...
  if (root_page_id_ == INVALID_PAGE_ID)
  {
    ...
    B_PLUS_TREE_LEAF_PAGE_TYPE *lp = reinterpret_cast<B_PLUS_TREE_LEAF_PAGE_TYPE *>(
        page->GetData());
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
bool BPLUSTREE_TYPE::InsertIntoLeaf(
    const KeyType &key,
    const ValueType &value,
    Transaction *transaction)
{
  assert(transaction == nullptr || (transaction->GetPageSet()->empty() && transaction->GetPageSet()->size() == 0));
  B_PLUS_TREE_LEAF_PAGE_TYPE *leaf = FindLeafPage(key, transaction, TreeOperation::Insert);
  if (leaf == nullptr)
  {
    return false;
  }
  ...
  if (new_size > leaf->GetMaxSize())
  {
    ...
    InsertIntoParent(left_leaf,
                     left_leaf->KeyAt(left_leaf->GetSize() - 1),
                     right_leaf);
  ...
  if (transaction)
  {
    ReleaseAllLatches(transaction, TreeOperation::Insert, true);
  }
  else
  {
    ...
    Transaction *transaction)
{
  ...
  if (parent_pid == INVALID_PAGE_ID)
  {
    ...
    BPlusTreeParentPage *parent = reinterpret_cast<BPlusTreeParentPage *>(
        page->GetData());
  ...
  {
    BPlusTreeParentPage *old_parent = parent;
    BPlusTreeParentPage *new_parent = Split(old_parent);
    InsertIntoParent(old_parent, new_parent->KeyAt(0), new_parent);
...
void BPLUSTREE_TYPE::Remove(const KeyType &key, Transaction *transaction)
{
  ...
  B_PLUS_TREE_LEAF_PAGE_TYPE *leaf = FindLeafPage(key, transaction, TreeOperation::Delete);
  assert(leaf != nullptr);
  ...
  if (transaction)
  {
    ReleaseAllLatches(transaction, TreeOperation::Delete, true);
  }
  else
  {
    buffer_pool_manager_->UnpinPage(leaf->GetPageId(), true);
    ...
    int index, Transaction *transaction)
{
  ...
  else
  {
    node->MoveAllTo(
        neighbor_node,
        parent->ValueIndex(neighbor_pid),
        buffer_pool_manager_,
        comparator_);
...
{
  KeyType key;
  B_PLUS_TREE_LEAF_PAGE_TYPE *page = FindLeafPage(key, nullptr, TreeOperation::Search, true);
    ...
    bool leftMost)
{
  ...
  assert(transaction == nullptr ||
         (transaction->GetPageSet()->empty() &&
          transaction->GetDeletedPageSet()->empty()));
  ...
  if (transaction)
  {
    if (op_type != TreeOperation::Search)
    {
      page->WLatch();
    }
    else
  ...
  while (!btree_page->IsLeafPage())
  {
    ...
    if (transaction)
    {
      ...
      else if (op_type == TreeOperation::Insert)
      {
      ...
      else if (op_type == TreeOperation::Delete)
      {
...
INDEX_TEMPLATE_ARGUMENTS
void BPLUSTREE_TYPE::ReleaseAllLatches(
    Transaction *transaction,
    TreeOperation op_type, bool dirty)
{
  assert(transaction);
  ...
  if (op_type == TreeOperation::Delete)
  {
...
/*
 * Print out the b+tree structure.
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
void BPLUSTREE_TYPE::InsertFromFile(
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
void BPLUSTREE_TYPE::RemoveFromFile(
    const std::string &file_name,
    Transaction *transaction)
{
  int64_t key;
